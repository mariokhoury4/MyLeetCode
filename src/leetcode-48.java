/**
 * 48. Rotate Image
 * You are given an n x n 2D matrix representing an image, rotate the image by 90 degrees (clockwise).
 *
 * You have to rotate the image in-place, which means you have to modify the input 2D matrix directly. DO NOT allocate another 2D matrix and do the rotation.
 *
 *
 *
 * Example 1:
 *
 *
 * Input: matrix = [[1,2,3],[4,5,6],[7,8,9]]
 * Output: [[7,4,1],[8,5,2],[9,6,3]]
 * Example 2:
 *
 *
 * Input: matrix = [[5,1,9,11],[2,4,8,10],[13,3,6,7],[15,14,12,16]]
 * Output: [[15,13,2,5],[14,3,4,1],[12,6,8,9],[16,7,10,11]]
 *
 *
 * Constraints:
 *
 * n == matrix.length == matrix[i].length
 * 1 <= n <= 20
 * -1000 <= matrix[i][j] <= 1000
 */
class Solution {
    /**
     * Rotate a matrix as follow:
     * i,j --> n-1-j,i
     * n-1-j,i --> n-i-1,n-j-1
     * n-i-1,n-j-1 --> j,n-i-1
     * j,n-i-1 --> i,j
     * @param rotated matrix
     */
    public void rotate(int[][] matrix) {
        // Length of the matrix
        final int matrixLength = matrix.length;

        // Check if we need to iterate to half or half + 1
        final int n = matrixLength % 2 == 0 ? matrixLength / 2 : (matrixLength / 2) + 1;

        for (int i = 0; i < matrixLength / 2; i++) {
            for(int j = 0; j < n; j++) {
                int temp = matrix[i][j];
                matrix[i][j] = matrix[matrixLength - j - 1][i];
                matrix[matrixLength - j - 1][i] = matrix[matrixLength - i - 1][matrixLength - j - 1];
                matrix[matrixLength - i - 1][matrixLength - j - 1] = matrix[j][matrixLength - i - 1];
                matrix[j][matrixLength - i - 1] = temp;
            }
        }
    }
}
